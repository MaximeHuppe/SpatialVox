...
`B(I)` is the difference between the model reading the image and redrawing a spatial prior. The §7 prompt-only ablation removes it and keeps everything else:
...
Whether that is the image's fault or the corpus's is measurable. A single global threshold recovers `labels > 0` at IoU **0.0796** on `data/mri` and **0.994** on the easy synthetic corpus — and on a corpus built to sit between them (0.27, shapes still plainly visible) transfer holds at 0.737 against a 0.1625 floor. So `B(I)` generalises exactly when the image carries a class-agnostic boundary, which is what §4 predicted.
