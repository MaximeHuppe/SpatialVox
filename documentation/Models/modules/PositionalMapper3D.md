...
`PositionalMapper3D(tau=0.5, min_mass=1e-6)`
...
The conjunction of three 45° cones is an **elongated wedge** and the target sits near its *apex*, so the region contains the target without indicating it. Two consequences, both in `docs/proposal/deviations.md` §3.1b:
