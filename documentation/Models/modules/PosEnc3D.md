...
Two instances, **no shared weights**.
...
Axis names match array order `(z, y, x)`, **not** RAS `(x, y, z)`. That is the opposite of `world_grid`'s channel stack.
...
4. **Not world coordinates.** No millimetres, no RAS, no `[-1,1]`. Learned offsets. `medial`/`lateral` against the volume centre plane live in `world_grid`, not here.
